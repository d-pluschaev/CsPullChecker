...
//        );
    ...
    protected function analyzeDiff($diff, $rep_dir = 'sugarcrm')
    {
        $diff = preg_split("/((\r?\n)|(\r\n?))/", $diff);
        $out = array();
        $line = $is_new = 0;
        foreach ($diff as $row) {
            if (preg_match("~^---\s(.*)~", $row, $matches)) {
                $path_minus = substr($matches[1], strpos($matches[1], $rep_dir));
                continue;
            } elseif (preg_match("~^\+\+\+\s(.*)~", $row, $matches)) {
                $path = substr($matches[1], strpos($matches[1], $rep_dir));
